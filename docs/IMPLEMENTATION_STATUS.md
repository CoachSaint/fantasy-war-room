...
- Migrations `0011`–`0012` write Yahoo decision and weekly-point prediction ledgers transactionally when recommendations are published, retain old recommendations on refresh, protect recorded facts against updates, and deny direct authenticated ledger writes. Migrations `0013`–`0015` preserve the forecast source and fresh Yahoo pregame proof, reject a late/previously observed game forecast from measured accuracy, store source-linked outcomes only after Yahoo advances the week, and deduplicate identical refresh forecasts in MAE/RMSE. The points outcome uses nflverse raw game stat columns under the imported Yahoo coefficients, never its generic PPR total as a custom-league result. A hosted disposable fixture passed expected three unique forecast errors (`-2`, `+3`, `-2`, MAE `2.333`, RMSE `2.380`), idempotent reconciliation, unverified late forecast refusal, owner/outsider/anonymous boundaries, and cleanup. This is synthetic acceptance; no real league outcome or calibration result exists. Snap/route/availability outcomes and calibration remain incomplete.
- Clean reviewed code commit `375cd2620a31cf303672be7a67b4404d48e8e771` was staged with `vercel deploy --prod --skip-domain --yes --meta candidate_sha=375cd2620a31cf303672be7a67b4404d48e8e771` as protected READY deployment `dpl_D7JgQYWWtHZTigoyZc69Ffdvwrpo` (`https://fantasy-war-room-bw97dueuo-coachsaints-projects.vercel.app`). The Vercel remote build passed, protected health returned HTTP 200 with hosted Supabase connected and demo mode true, and anonymous History returned HTTP 401. CLI deployment SHA binding is operator-verified from the clean checkout and command; Vercel supplied no independent Git source metadata. Read-only inspect after staging confirmed the public `fantasy-war-room-pi.vercel.app` alias remains on fallback `dpl_E8FFiYzfonqeR4npvGGQqg2jYYtg`. Astra's focused review of that commit found no P0/P1 in the corrected shared-league/ledger path and passed a protected preview, but held full V2/production acceptance. The clean branch was pushed to its own remote at docs commit `c14ca4e11a3f9482a8f996b6f84697be854cfbc4`; PR #2 was not updated or merged. The outcome changes in this worktree have not yet been reviewed, pushed, or staged. No public alias promotion or Yahoo refresh activation occurred.
...
| Schema | `0001` through `0015` are aligned on the dedicated project; `0006` creates no refresh job; private full schema/data dumps predate `0005`, and later additive migrations have read-only metadata/count checks and disposable hosted fixtures | Recheck history before promotion and prove full user/membership isolation with real league data |
