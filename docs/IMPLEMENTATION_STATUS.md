...
- Local and hosted migrations `0001`–`0016` are aligned. Migration `0010` serializes imports of a shared Yahoo league across managers and preserves a confirmed roster owner against stale writes. A hosted three-user fixture passed two-manager league mapping, concurrent lease refusal, owner preservation, owner-theft refusal, and outsider lease refusal. The fixture used synthetic Yahoo data, not owner OAuth.
...
| Schema | `0001` through `0016` are aligned on the dedicated project; `0006` creates no refresh job; private full schema/data dumps predate `0005`, and later migrations have read-only metadata/count checks and disposable hosted fixtures | Recheck history before promotion and prove full user/membership isolation with real league data |
...
| Vercel | Cron declaration, healthy public demo fallback, and protected READY corrected stage `dpl_Gp385gXRQvGRp6Sc6NMqoS3YvqNL` at code commit `db154992...` exist; the user-facing alias remains on fallback | Exact final promoted deployment binding, cron history, auth negative/positive checks |
