...
- Scout remains degraded: Yahoo consent/league sync and snapshot diff are incomplete. The local candidate now computes narrow same-position Yahoo lineup swaps from current Sleeper stat forecasts and imported Yahoo scoring modifiers, with source evidence. A disposable hosted-database control fixture proved one owner-scoped recommendation and idempotent replacement; it was cleaned up. This does not establish forecast accuracy or a real Yahoo import. No real user league or recommendation has been imported or created.
...
| Materialization | Candidate branch can persist source-labeled global nflverse actuals, matched evidence, and separate current-week Sleeper forecasts; a narrow Yahoo same-position lineup materializer passed a disposable hosted-database control fixture; snapshot diff remains skipped | Repeatable live Scout run with real Yahoo consent, exact league scoring support, and source-backed actionable recommendations |
